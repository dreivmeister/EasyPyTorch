...
class PSPNet(nn.Module):
    def __init__(self, num_classes, input_dim, cnn_backbone, bin_sizes=[1,2,3,6]):
        super().__init__()
        #cnn backbone is a pretrained cnn for feature extraction
        
        #two parts:
        #1. CNN (ResNet type for feature extraction, provides Feature Map)
        self.CNN = cnn_backbone # ResNet18(image_channels=input_dim[0])
        with torch.no_grad():
